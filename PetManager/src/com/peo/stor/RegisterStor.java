...
{
	EditText storRegister, password, surePassword, name, address, phone,
			number, qq, email, startTime, stopTime, anotherServer;
	CheckBox phoneBox, numberBox, qqBox, emailBox;
	CheckBox hospBox, foodBox, hairdressingBox, sellBox, applianceBox,
			travelBox, anotherBox;
	...
	public void showDialog(Context context)
	{
		...
		builder.setNeutralButton("立即登录", new DialogInterface.OnClickListener()
		{
			public void onClick(DialogInterface dialog, int whichButton)
			{
				...
				Intent intent = new Intent(RegisterStor.this,
						LoginActivity.class);
		...
		name.setOnFocusChangeListener(new OnFocusChangeListener()
		{
			...
			public void onFocusChange(View v, boolean hasFocus)
			{
				...
				if (!name.isFocused())
				{
					...
					else if (!JudgeFormat.isLogical(name.getText().toString()))
					{
		...
		password.setOnFocusChangeListener(new OnFocusChangeListener()
		{
			...
			public void onFocusChange(View v, boolean hasFocus)
			{
				...
				if (!password.isFocused())
				{
					...
					else if (!JudgeFormat.isPassword(password.getText()
							.toString()))
					{
		...
		number.setOnFocusChangeListener(new OnFocusChangeListener()
		{
			...
			public void onFocusChange(View v, boolean hasFocus)
			{
				...
				if (!number.isFocused())
				{
					...
					else if (!JudgeFormat.isQQNumber(number.getText()
							.toString()))
					{
	...
	private void loadSpinner()
	{
		...
		province_spinner.setOnItemSelectedListener(new OnItemSelectedListener()
		{
					...
					int arg2, long arg3)
			{
				...
				strProvince = province_spinner.getSelectedItem().toString();// 得到选择的内容，也就是省的名字
